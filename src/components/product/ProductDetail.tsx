...
const ProductDetail: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div 
        ref={stickyRef}
        className={`fixed bottom-0 left-0 right-0 bg-surface border-t border-border p-4 z-40 md:hidden transition-transform duration-200 ${
        ...
        <div className="container mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <span className="text-2xl font-bold text-accent">${mockProduct.price}</span>
              {mockProduct.original_price > mockProduct.price && (
                <span className="text-sm text-text-dim line-through">${mockProduct.original_price}</span>
              )}
            </div>
            <Button 
              onClick={handleAddToCart}
