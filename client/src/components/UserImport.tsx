    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text_dim mt-1">
          ...
          name, email, role (customer, seller, admin), phone, profile_picture_url.
        </p>
      </div>

      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="user-file">CSV File</Label>
          <Input
            id="user-file"
            type="file"
            accept=".csv"
      ...
      {result && (
        <div className={`p-4 rounded-md ${result.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
          ...
          {result.count && (
            <p className="mt-1">Successfully imported {result.count} users.</p>
          )}
